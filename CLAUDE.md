...
- Units are **explicit** everywhere: mg/dL and mmol/L are both real-world outputs — never assume one. Carry the unit alongside the value; convert at the edge, not in the middle.
- Timestamps are **unambiguous**: store UTC, render local, and say which is which. A silent timezone shift makes a reading wrong, not just ugly.
